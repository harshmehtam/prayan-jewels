...
export default function OrderDetailsPage() {
  ...
  const formatAddress = (order: OrderWithItems, type: 'shipping' | 'billing') => {
    ...
    return [
      `${order[`${prefix}FirstName` as keyof OrderWithItems]} ${order[`${prefix}LastName` as keyof OrderWithItems]}`,
      order[`${prefix}AddressLine1` as keyof OrderWithItems],
      order[`${prefix}AddressLine2` as keyof OrderWithItems],
      `${order[`${prefix}City` as keyof OrderWithItems]}, ${order[`${prefix}State` as keyof OrderWithItems]} ${order[`${prefix}PostalCode` as keyof OrderWithItems]}`,
      order[`${prefix}Country` as keyof OrderWithItems]
    ].filter(Boolean).join('\n');
  };
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <div className="border-t border-gray-200 mt-6 pt-6 space-y-3">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-900">₹{(order.tax || 0).toLocaleString('en-IN')}</span>
            </div>
            <div className="flex justify-between text-sm">
